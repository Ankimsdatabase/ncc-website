    ...
    if ($result->num_rows > 0) {
       ...
       <tbody>';
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<td>
                <a href="../event_detail.php?id=' . $row["id"] . '" target="_blank" class="btn btn-primary mr-3"><i class="fa fa-eye" aria-hidden="true"></i></a>
                <form action="edit_event.php" method="POST" class="d-inline">
                    <input type="hidden" name="id" value=' . $row["id"] . '><button type="submit" class="btn btn-info mr-3" name="view" value="View"><i class="fas fa-pen"></i></button>
                </form>
        ...
        $sql = "SELECT COUNT(*) AS total_records FROM event";
...
<div><a class="btn btn-danger box" href="./add_event.php"><i class="fas fa-plus fa-2x"></i></a></div>
</div>
